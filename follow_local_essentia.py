#!/usr/bin/env python3
"""Follow an atomically growing manifest and analyze full tracks with Essentia."""
...
from cloud_audio_full import append, completed, run_essentia
...
def import_new(results: Path, manifest: Path, state: Path) -> None:
    command = [
        ...
        str(ROOT / "import_full_audio_results.py"),
...
def main() -> None:
    ...
    while True:
        ...
        pending = [row for row in rows if (row["spotify_id"], "essentia_full") not in done
        ...
        for row in pending:
            ...
            try:
                result = run_essentia(row, "cpu")
                payload = {"spotify_id": row["spotify_id"], "stage": "essentia_full",
            ...
            print(f"essentia-follow {row['spotify_id']} {payload['status']}", flush=True)
            if imported_since >= 25:
                import_new(args.output, args.manifest, args.state)
                imported_since = 0
        done = completed(args.output)
        successful = sum((row["spotify_id"], "essentia_full") in done for row in rows)
        if imported_since and not pending:
            import_new(args.output, args.manifest, args.state)
            imported_since = 0
        print(f"essentia-follow ready={len(rows)} successful={successful}", flush=True)
