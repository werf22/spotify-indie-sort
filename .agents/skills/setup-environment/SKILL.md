...
description: "Use to build a durable working environment for a project instead of starting every chat from scratch — a AGENTS.md with working rules, an always-do / ask-first / never-do guardrail split, optional tool-level hooks for hard rules, and pointers to a knowledge base. Trigger when setting up a new project/repo, when the user wants Codex to stop re-explaining preferences each time, wants guardrails/rules/'pravidlá', wants to set up AGENTS.md, or types /setup-environment."
---
...
### A) AGENTS.md — pravidlá práce

`AGENTS.md` sa Claudovi automaticky injektuje pri každom prompte. Je to prvá vec, ktorú prečíta. Použi tento základný template a priprav ho na mieru projektu:
...
Do AGENTS.md tiež stručne popíš: ako je repo/workspace usporiadané, aké skilly máš a kedy ich použiť, kde leží znalostná báza, a kľúčové pravidlá.
...
Pravidlo v AGENTS.md je *žiadosť* — AI ho môže ignorovať. Pri kritických veciach to musí byť vynútené na úrovni nástroja: napr. `PreToolUse` hook, ktorý zablokuje `Edit`/`Write` na chránenom priečinku. Na nastavenie takéhoto hooku použi skill **`/update-config`** (spravuje `settings.json`).
...
Po behu tohto skillu má projekt: hotový `AGENTS.md`, jasné always/ask/never buckety, (voliteľne) hook na kritické súbory a odkaz na znalostnú bázu. Zmeny v `AGENTS.md` a hookoch mi pred zápisom daj odsúhlasiť.
