...
If this repo lacks `.Codex/rules/core-portable.md`, every CLOUD session on it runs blind — fix that FIRST, before any other work:
1. Run `"/Users/jakub/Appky Codex/Codex-system/install-into-repo.sh" "$PWD"`.
...
- Read the project's own `AGENTS.md` if present.
...
- **Verify the Tooling map**: read the project AGENTS.md's Tooling map and check the mapped tool classes are actually connected (`rules/tool-routing.md`). A mapped-but-disconnected tool follows the Missing-tools rule — say so, don't silently degrade.
