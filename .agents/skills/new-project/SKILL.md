...
## 3. Scaffold project docs from `~/.Codex/templates/`

Create in the project root, filled in — not blank copies:

| File | Fill with |
|---|---|
| `AGENTS.md` | Stack table from step 2, Tooling map (tool class → connected server/CLI, `rules/tool-routing.md`), conventions, budgets (cost/perf if known), project guardrails, Docs map naming all the files below |
...
Then vendor the portable AI layer — cloud sessions depend on it: run `"/Users/jakub/Appky Codex/Codex-system/install-into-repo.sh" .` (adds `.Codex/rules/` incl. `core-portable.md`, in-repo skills, the protect-paths hook, and the AI Layer import block in AGENTS.md).
...
- Create `.Codex/protected-paths.txt` in the project: migrations, production config, `.env`, plus anything the spec marks as untouchable. One pattern per line.
- Offer to wire the `protect-paths` hook (`~/.Codex/hooks/README.md`) so the NEVER tier is enforced at tool level, not just requested in text (core §5).
