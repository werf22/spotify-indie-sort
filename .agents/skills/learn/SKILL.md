...
- **knowledge** — not a rule but a fact: narrative context (who/what/why) with nothing to violate → the matching `~/.Codex/knowledge/` file (e.g. `personal/preferences.md`).
...
Grep `~/.Codex/rules/learned-rules.md` (ALL sections — Seed rules, Learned rules, Declined) and the project lessons file for keywords from the candidate rule. Rules are stored in English (§5), so translate the keywords to English before grepping. If an overlapping rule exists, skip the save question — say in one line that it's already covered and propose **updating or sharpening it in place** instead of adding a near-duplicate. Two rules saying almost the same thing is how rule files rot. A hit under `## Declined` kills the candidate outright: the user already said no — drop it without asking.
...
- **Cloud/ephemeral session check (BEFORE any global write):** if `~/.Codex/rules/learned-rules.md` does not exist, this is not the user's machine (cloud sandbox) — a global write would evaporate with the sandbox. Save the candidate to the **project lessons file** instead, tagged `[global-candidate]`, and tell the user to promote it locally later. Local sessions: when the project lessons file contains `[global-candidate]` tags, propose promoting them to `learned-rules.md` at session start.
- **global** → append to `~/.Codex/rules/learned-rules.md` under `## Learned rules`:
  `- [YYYY-MM-DD] rule — why`
  If §3 matched an existing rule under `## Seed rules`, edit it in place there — never append a near-duplicate under `## Learned rules`.
  Then run `"/Users/jakub/Appky Codex/Codex-system/sync-global.sh"` (autopilot) — it pushes the updated rules to the canonical bundle repo, and the sync-ai-layer GitHub Action fans the fresh snapshot out to every repo, so cloud sessions learn the rule within minutes. (Not applicable in cloud sandboxes — the cloud check above already routed the save to project lessons.)
- **project** → append to the project's lessons file in the same format; if none exists, create it from `~/.Codex/templates/LESSONS.md`. Then verify the project AGENTS.md contains an `@LESSONS.md` import (adjusted to the actual filename); if the line is missing, add it; if no project AGENTS.md exists, create a minimal one containing the import and say so. A lessons file without the import line does not load — confirm the import as part of the save confirmation (§6).
- **knowledge** → append to the matching `~/.Codex/knowledge/` file (see routing rule, §2).
...
| Signal | Escalation |
|---|---|
| Path-based NEVER rule ("never touch <path>") | Add a pattern to `~/.Codex/protected-paths.txt` (global) or `<project>/.Codex/protected-paths.txt`, then verify the protect-paths hook is installed and active per `~/.Codex/hooks/README.md` (run the manual test there) — the pattern file alone enforces nothing |
| Other safety-critical NEVER rule (core §5) that is mechanically checkable at the tool layer (a path, a command, a tool-call pattern) | Text is a request, not a rule — propose a hook to enforce it physically; see `~/.Codex/hooks/README.md` |
