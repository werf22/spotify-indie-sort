#!/usr/bin/env python3
"""Run the BITRATE validation probe on one bounded pod. NO database import.

Analyses 25 already-analysed tracks whose clips were re-encoded at 96 kbps and
downloads results.jsonl for validate_bitrate.py --compare. Results are
deliberately never imported: they would overwrite production artifacts computed
from 192 kbps clips, destroying the very baseline being tested.

Detached remote execution + marker polling per the standing rule: a local network
drop cannot kill the paid run.
"""
...
# One pass, all four stages, over clips encoded at 96 kbps instead of 192. The
# 192 kbps answers are already in the database, so this single run is the whole
# cost of the experiment. Rhythm matters most: a BPM that moves is a wrong
# number in a DJ library, not a tolerable quality trade.
python cloud_audio_full.py --manifest data/bitrate_probe/manifest.csv --output data/bitrate_probe/results.jsonl --stage all --device cuda
...
def create_pod() -> str:
    ...
    for gpu in rp.GPU_CANDIDATES:
        proc = rp.run([
            ...
            "--name", "probe-bitrate-musicdb", "--public-ip", "--ssh", "--ports", "22/tcp",
...
def run_probe() -> None:
    ...
    try:
        ...
        for name in ("results_fp32cap.jsonl", "results_fp16cap.jsonl"):
            rp.run(scp + [f"{target}:/workspace/data/bitrate_probe/{name}",
